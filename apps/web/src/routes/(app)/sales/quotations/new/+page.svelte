...
async function handleSubmit(e: Event) {
  ...
  try {
    ...
    goto(`/sales/quotations/${quotation.id}`);
  } catch {
...
<div class="space-y-6">
  ...
  <form onsubmit={handleSubmit} class="space-y-6">
    <div class="rounded-lg border bg-card p-6 shadow-sm">
      ...
      <div class="grid gap-4 md:grid-cols-2">
        <div>
          ...
          <input id="customerId" type="text" bind:value={customerId} class="w-full rounded-md border bg-background px-3 py-2 text-sm" placeholder="Customer ID" required />
        </div>
        <div>
          <label for="validUntil" class="mb-1 block text-sm font-medium text-card-foreground">Valid Until</label>
          <input id="validUntil" type="date" bind:value={validUntil} class="w-full rounded-md border bg-background px-3 py-2 text-sm" />
        </div>
      </div>
      <div class="mt-4">
        <label for="notes" class="mb-1 block text-sm font-medium text-card-foreground">Notes</label>
        <textarea id="notes" bind:value={notes} rows="3" class="w-full rounded-md border bg-background px-3 py-2 text-sm" placeholder="Optional notes"></textarea>
      </div>
    </div>
    ...
    <div class="flex items-center justify-end gap-3">
      <a href="/sales/quotations" class="inline-flex items-center rounded-md border px-4 py-2 text-sm font-medium hover:bg-accent">Cancel</a>
