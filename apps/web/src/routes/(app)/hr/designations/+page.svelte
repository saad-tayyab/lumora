...
<div class="space-y-6">
  ...
  <form onsubmit={handleCreate} class="rounded-lg border bg-card p-6 shadow-sm">
    ...
    <div class="grid gap-4 md:grid-cols-4">
      <div><label for="title" class="mb-1 block text-sm font-medium">Title *</label><input id="title" type="text" bind:value={title} class="w-full rounded-md border bg-background px-3 py-2 text-sm" required /></div>
      ...
      <div><label for="level" class="mb-1 block text-sm font-medium">Level</label><input id="level" type="number" bind:value={level} class="w-full rounded-md border bg-background px-3 py-2 text-sm" min="1" /></div>
    </div>
