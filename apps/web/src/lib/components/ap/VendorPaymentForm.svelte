...
let {
  vendors,
  errors = {},
}: { vendors: Array<{ id: string; name: string }>; errors?: Record<string, string[]> } = $props();
...
<div class="rounded-lg border bg-card p-6 shadow-sm">
	<form method="POST" class="space-y-6">
		...
		<div class="flex items-center gap-3">
			...
			<a href="/ap/payments" class="rounded-md border px-4 py-2 text-sm font-medium text-card-foreground hover:bg-accent">Cancel</a>
		</div>
	</form>
</div>
