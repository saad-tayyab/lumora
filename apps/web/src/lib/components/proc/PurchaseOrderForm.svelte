...
let {
  ...
  purchaseOrder?: { vendorId: string; expectedDeliveryDate: string | null; notes: string | null };
  errors?: Record<string, string[]>;
} = $props();

let vendorId = $state(purchaseOrder?.vendorId ?? '');
let expectedDeliveryDate = $state(purchaseOrder?.expectedDeliveryDate ?? '');
let notes = $state(purchaseOrder?.notes ?? '');
let lineItems = $state<
	...
	<form method="POST" class="space-y-6">
		<div class="grid gap-4 md:grid-cols-2">
			...
			<div>
				...
				<input id="expectedDeliveryDate" name="expectedDeliveryDate" type="date" bind:value={expectedDeliveryDate} class="mt-1 block w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground focus:border-ring focus:outline-none focus:ring-1 focus:ring-ring" />
			</div>
		</div>
		...
		<div>
			...
			<textarea id="notes" name="notes" rows="3" bind:value={notes} class="mt-1 block w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-ring focus:outline-none focus:ring-1 focus:ring-ring" placeholder="Optional notes"></textarea>
		</div>
		...
		<div class="flex items-center gap-3">
			<button type="submit" disabled={isSubmitting} class="rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-50">
				{#if isSubmitting}Creating...{:else}Create Purchase Order{/if}
			</button>
			<a href="/proc/purchase-orders" class="rounded-md border px-4 py-2 text-sm font-medium text-card-foreground hover:bg-accent">Cancel</a>
		</div>
	</form>
</div>
