...
<div class="rounded-lg border bg-card p-6 shadow-sm">
	<form method="POST" class="space-y-6">
		<div class="grid gap-4 md:grid-cols-2">
			...
			<div>
				<label for="creditNoteNumber" class="block text-sm font-medium text-card-foreground">Credit Note Number *</label>
				<input id="creditNoteNumber" name="creditNoteNumber" type="text" required bind:value={creditNoteNumber} class="mt-1 block w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-ring focus:outline-none focus:ring-1 focus:ring-ring" placeholder="CN-001" />
			</div>
			<div>
		...
		<div>
			...
			<input id="reason" name="reason" type="text" required maxlength="500" bind:value={reason} class="mt-1 block w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-ring focus:outline-none focus:ring-1 focus:ring-ring" placeholder="Reason for credit note" />
		</div>
		...
		<div class="flex items-center gap-3">
			...
			<a href="/ar/credit-notes" class="rounded-md border px-4 py-2 text-sm font-medium text-card-foreground hover:bg-accent">Cancel</a>
		</div>
	</form>
</div>
