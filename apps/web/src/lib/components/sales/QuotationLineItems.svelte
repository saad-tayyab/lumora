...
let {
  lineItems,
  onChange,
}: { lineItems: QuotationLineItem[]; onChange: (items: QuotationLineItem[]) => void } = $props();
	...
	{#each lineItems as item, index}
		<div class="grid items-end gap-3 rounded-md border p-3 md:grid-cols-[1fr_2fr_1fr_1fr_1fr_auto]">
			...
			<div>
				...
				<input type="number" value={item.discount} oninput={(e) => updateLine(index, 'discount', (e.target as HTMLInputElement).value)} min="0" step="0.01" class="mt-1 block w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground focus:border-ring focus:outline-none focus:ring-1 focus:ring-ring" />
			</div>
