...
let {
  ...
  salesOrder?: { customerId: string; expectedDeliveryDate: string | null; notes: string | null };
  errors?: Record<string, string[]>;
} = $props();

let customerId = $state(salesOrder?.customerId ?? '');
let expectedDeliveryDate = $state(salesOrder?.expectedDeliveryDate ?? '');
let notes = $state(salesOrder?.notes ?? '');
let lineItems = $state<
  Array<{
    itemId: string;
    description: string;
	...
	<form method="POST" class="space-y-6">
		...
		<div class="flex items-center gap-3">
			<button type="submit" disabled={isSubmitting} class="rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-50">
				{#if isSubmitting}Creating...{:else}Create Sales Order{/if}
			</button>
			<a href="/sales/orders" class="rounded-md border px-4 py-2 text-sm font-medium text-card-foreground hover:bg-accent">Cancel</a>
		</div>
	</form>
</div>
